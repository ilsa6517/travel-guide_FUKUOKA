...
(function(){'use strict';
 function init(){
  ...
  function sortReservations(mode){
   ...
   try{localStorage.setItem('fukuoka-2026-reservation-sort',mode)}catch(e){}
  }
  ...
  var mode='priority';try{if(localStorage.getItem('fukuoka-2026-reservation-sort')==='date')mode='date'}catch(e){}sortReservations(mode);
  document.querySelectorAll('[data-reservation-id]').forEach(function(select){
   var key='fukuoka-2026-audit:'+select.dataset.reservationId;
   ...
   select.addEventListener('change',function(){try{localStorage.setItem(key,select.value)}catch(e){select.title='此瀏覽器無法保存狀態，請自行記錄'}});
  });
 }
