...
const diningPlan=$('#food .dining-plan');if(diningPlan&&diningPlan.tagName!=='DETAILS'){
 ...
 fold.innerHTML='<summary><b>這次的餐桌</b><span>展開 ＋</span></summary><div class="dining-plan-body"></div>';
...
const stay=$('#stay');if(stay&&!$('.travel-info-fold',stay)){
 ...
 if(shell){const heading=$(':scope>.section-heading',shell),fold=document.createElement('details');fold.className='travel-info-fold';
  fold.innerHTML='<summary><span><small>FLIGHT & STAY</small><b>查看航班與住宿</b><em>交通與住宿按已確認資料展示</em></span><i aria-hidden="true"></i></summary><div class="travel-info-body"></div>';
...
all('.audit-reservation').forEach(row=>{
 ...
 const id=select.dataset.reservationId,key='fukuoka-2026-check:'+id,legacy=(()=>{try{return localStorage.getItem('fukuoka-2026-audit:'+id)}catch{return null}})();
 const label=document.createElement('label');label.className='reservation-check';const check=document.createElement('input');check.type='checkbox';check.checked=read(key,legacy?legacy==='已預約，有憑證':select.value==='已預約，有憑證');
 const text=document.createElement('span');label.append(check,text);select.closest('label').replaceWith(label);
 const sync=()=>{const state=check.checked?'已處理':'待處理';text.textContent=state;row.classList.toggle('reservation-done',check.checked);check.setAttribute('aria-label',title+' '+state)};sync();
 if(cloud){
 ...
 }else check.onchange=()=>{if(!save(key,check.checked)){check.checked=!check.checked;alert('本機保存失敗，請檢查瀏覽器存儲設置。')}sync()};
});
const sorter=$('[data-reservation-sort][aria-pressed=true]');if(sorter)sorter.click();
const heading=$('.audit-reservations>p');if(heading)heading.textContent='必须：固定項目與交通；建议：重點正餐；随缘：按體力和心情。'+(cloud?'預約勾選由同行成員共享；個人行李清單仍保存在各自手機。':'處理好後打勾，狀態保存在本機。');
if(cloud&&heading){const refresh=document.createElement('button');refresh.type='button';refresh.className='cloud-refresh';refresh.textContent='刷新共享狀態';heading.after(refresh);refresh.onclick=async()=>{refresh.disabled=true;try{await cloud.refresh();refresh.textContent='已同步'}catch(e){refresh.textContent='連接失敗，點擊重試'}finally{refresh.disabled=false}};cloud.refresh().catch(()=>{refresh.textContent='尚未連接，點擊重試'})}
...
const tipsHeading=$('#tips .section-heading');if(tipsHeading){const legend=document.createElement('p');legend.className='tips-priority-legend';legend.textContent='必须：安全、禮儀與固定時間　／　建议：提前留意更省心　／　随缘：按體力與習慣調整';tipsHeading.after(legend);}
...
all('#words .vocab-items>div,#words .phrase-grid>div').forEach(row=>{
 const target=$('b',row);if(!target)return;const value=target.textContent.trim();const button=document.createElement('a');button.href='#';button.className='speech-link';button.textContent=value;button.setAttribute('aria-label','朗讀 '+value);button.title='點擊朗讀，再次點擊停止';
 button.onclick=(event)=>{event.preventDefault();if(!('speechSynthesis'in window)){speechStatus.textContent='此瀏覽器不支持朗讀，請使用支持系統語音的瀏覽器。';return}
 ...
 const group=row.closest('details')?.querySelector('summary')?.textContent||'';const lang=row.closest('.language-edition')?.querySelector('h3')?.textContent==='英語備用'?'en-US':window.HANDBOOK_CONFIG.language;const u=new SpeechSynthesisUtterance(value);u.lang=lang;u.rate=.85;
 const voice=speechSynthesis.getVoices().find(v=>v.lang.toLowerCase().startsWith(lang.slice(0,2)));if(voice)u.voice=voice;
 speechStatus.textContent='使用設備語音朗讀；離線可用性取決于已安裝語音。';speaking=button;button.classList.add('is-speaking');u.onend=()=>{button.classList.remove('is-speaking');if(speaking===button)speaking=null};u.onerror=()=>{u.onend();speechStatus.textContent='未能播放，請檢查設備音量和系統語音設置。'};speechSynthesis.speak(u);
