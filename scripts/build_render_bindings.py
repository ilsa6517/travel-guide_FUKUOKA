...
def sight_card(place: dict) -> str:
    ...
    return (
        ...
        f'<b>{e(marker)}</b><div class="sight-facts"><span>建議停留 {e(place.get("duration_minutes"))} 分钟</span></div><p>{e(place.get("description", ""))}</p>'
...
def checklist(items: list, css: str) -> str:
    ...
    for index, item in enumerate(items, 1):
        ...
        priority_label = {'必须': '必須', '建议': '建議', '随缘': '隨緣'}.get(priority, priority)
        level = {'必须': 'must', '建议': 'recommended', '随缘': 'optional'}.get(priority, 'recommended')
        badge = f'<em class="packing-priority is-{level}">{e(priority_label)}</em>'
...
def booking(profile: dict) -> str:
    ...
    blocks = (
        f'<div class="prepare-block"><div class="prepare-heading"><span>01</span><div><h3>必備物品</h3><p>按必須、建議、隨緣排序，依序準備。</p></div></div>{checklist(sorted(model["essentials"], key=lambda item: {"必须": 0, "建议": 1, "随缘": 2}.get(item.get("priority"), 1)), "packing-checklist")}</div>'
        f'<div class="prepare-block"><div class="prepare-heading"><span>02</span><div><h3>需要預約與確認</h3><p>按必須、建議、隨緣排序，依序確認。</p></div></div>{checklist(model["confirm_ahead"], "booking-checklist")}</div>'
    )
...
def main() -> int:
    ...
    if profile.get("text_locale") == "zh-TW":
        ...
        fragments = {selector: preserve_profile_values(markup, profile) for selector, markup in fragments.items()}
